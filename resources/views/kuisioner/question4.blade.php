...
<body class="min-h-screen bg-gray-400">
    ...
    <div class="flex flex-col gap-y-5 items-center p-5">
        ...
        <form action="{{ route('user.kuisionerSessionFourStore') }}" method="post"
            class="bg-white rounded-xl p-5 w-1/2 mx-auto flex flex-col gap-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="flex justify-between items-center mt-5 px-5">
                    <input type="radio" name="question-{{ $data->id }}" value="1"
                        {{ isset($answer["question-$data->id"]) && $answer["question-$data->id"] == 1 ? 'checked' : ''}}
                        class="radio radio-primary" required />
                    <input type="radio" name="question-{{ $data->id }}" value="2"
                        {{ isset($answer["question-$data->id"]) && $answer["question-$data->id"] == 2 ? 'checked' : ''}}
                        class="radio radio-primary" required />
                    <input type="radio" name="question-{{ $data->id }}" value="3"
                        {{ isset($answer["question-$data->id"]) && $answer["question-$data->id"] == 3 ? 'checked' : ''}}
                        class="radio radio-primary" required />
                    <input type="radio" name="question-{{ $data->id }}" value="4"
                        {{ isset($answer["question-$data->id"]) && $answer["question-$data->id"] == 4 ? 'checked' : ''}}
                        class="radio radio-primary" required />
                    <input type="radio" name="question-{{ $data->id }}" value="5"
                        {{ isset($answer["question-$data->id"]) && $answer["question-$data->id"] == 5 ? 'checked' : ''}}
                        class="radio radio-primary" required />
                </div>
            </div>
            @endforeach
